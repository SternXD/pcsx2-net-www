...
Nearly six months after the release of PCSX2 2.6.0 and two years after the big release of 2.0.0, we are excited to unveil PCSX2 2.8.0! We're even more excited to showcase the things that we have been working on.

To start with a teaser, we have some serious performance gains to talk about with this release, seen here with Need for Speed: Carbon!
picture:
...
We have updated our system requirements again to keep up with several improvements to the core and graphics pipeline which demand more power than our old requirements specified.
...
We've also hand-picked the most popular codecs around. Previously we included everything that FFmpeg offers, but this was simply too much wasted space on codecs that are not frequently used, if at all. The reduced set of codecs should still provide you all the modern, efficient options you need without taking up much extra space.
...
- Hardware Encoders:
  ...
  - Media Foundations (H264, H265, AV1)
  - Vulkan Video (H264, H265, AV1)

- Software Encoders:
  - [libx264 (H264)](https://codecs.wiki/docs/encoders/x264)
  - [libsvtav1 (AV1)](https://codecs.wiki/docs/video/AV1)
...
Alongside that they have added Olympus Camera and Olympus Turbo MO miniEX IV (MO644U2S, MO133U2S) Storage Devices.
...
- The collapsing headers have been replaced with a sidebar in the achievement menu for games with multiple subsets.
...
:::info
Unlike other operating systems, Windows only allows icon files `(*.ico)` for shortcut icons.
:::
...
The status bar has received quite a number of quality of life improvements. You can now control the volume of the emulated virtual console directly from the main interface without going through the audio settings manually every time you want to adjust the volume. You can also now toggle the speed limit directly without using the hotkeys. Additionally, it shows you the host GPU usage, the renderer that the automatic renderer selection picked, and the currently active internal resolution multiplier.
...
A few new options have been added to the Game List's context menu that allow you to open the folders of various PCSX2-related data or resources right from within the application itself! You can access this by right-clicking a game entry on the Qt Game List.
...
You can now override the default data folder location that PCSX2 uses with your own path, instead of being stuck with the default locations or portable mode.

<PCSX2PRLink authors="SternXD,kamfretoz,TellowKrinkle,jasaaved">
...
Previously depth textures being moved in the texture cache would be done via GPU copy operations. Instead, shader copy/blit operations are now used. This ensures accuracy matches OpenGL and Vulkan, but avoids issues with Direct3D 11 which does not support partial copies, and Direct3D 12 having inconsistent driver-level support from GPU vendors.
...
Avoiding framebuffer switching has been further optimized. This allows us to reuse the old framebuffer attachments which provides a FPS boost.

To make it a bit simpler, the less re-binding/re-attachments are done the better. However, great care is needed because if done improperly it can cause issues.
...
Channel shuffles specifically always just requested entire targets to have valid information when looking up the source, however in some cases like with MotoGP, the source of the shuffle is a 24-bit target, so there is no valid alpha, so the lookup was failing, causing the shuffle to completely fall over. This corrects that by pre-checking what channels the shuffle requires for the read.
...
<SliderCompare
  before={require("./img/comparisons/13795/NFSUnderground2_Before.webp").default}
...
Improves GS memory clear behavior when a clear isn't immediately followed by a draw, making sure unwritten areas in GS memory are properly cleared instead of being left full of garbage data.

Target wrapping around memory was also improved for render targets and display targets. This fixes flashing shadows in *Beyond Good & Evil*, shadow bugs in *Ford Racing 2*, the menu in *Black*, and FMV playback in 60Hz *Silent Hill 3*.
...
<PCSX2PRLink authors="refractionpcsx2" prNums="14490">
  ### GS/TC: Fix target combining, scale and valid areas
</PCSX2PRLink>

Fixes an upscaling bug in `CombineAlignedTargets` where resolution scale was mistakenly applied to normalized source coordinates, causing the wrong data to be copied.

The valid draw area is also now preserved when combining targets. Previously, if a target was resized right after being combined, the missing draw area caused the new data to get lost. Fixing this resolves broken lighting in *Area 51* and misaligned blur effects in *Valkyrie Profile 2: Silmeria*.

Area 51:

<SliderCompare
  before={require("./img/comparisons/14490/Area51_Before.webp").default}
  after={require("./img/comparisons/14490/Area51_After.webp").default}
/>

Valkyrie Profile 2:

<SliderCompare
  before={require("./img/comparisons/14490/VP2_1_Before.webp").default}
  after={require("./img/comparisons/14490/VP2_1_After.webp").default}
/>

<PCSX2PRLink authors="refractionpcsx2" prNums="14492">
  ### GS/HW: Downscale target before 8bit conversion
</PCSX2PRLink>

Downscales selected render targets to native resolution with filtering before converting them to 8-bit paletted textures.

The 8-bit conversion routine doesn't do a great job on upscaled targets, leading to noisy dithering and color artifacts. Downscaling the target with filtering first yields a much cleaner result for post-processing effects in *Valkyrie Profile 2: Silmeria* and *Xenosaga Episode III*.

Valkyrie Profile 2:

<SliderCompare
  before={require("./img/comparisons/14492/VP2_Before.webp").default}
  after={require("./img/comparisons/14492/VP2_After.webp").default}
/>

Xenosaga Episode 3:

<SliderCompare
  before={require("./img/comparisons/14492/Xenosaga3_Before.webp").default}
  after={require("./img/comparisons/14492/Xenosaga3_After.webp").default}
/>

<PCSX2PRLink authors="refractionpcsx2" prNums="13823">
  ### GS: Fix copy/paste typo in TextureMinMax calculation, clean up code
</PCSX2PRLink>

Fixes a typo in the `TextureMinMax` calculation where vertical (Y) coordinates were mistakenly used to determine if horizontal (X) texture coordinates were flipped.

This error caused texture clipping routines (primarily in the Software renderer) to clip the wrong side of flipped 2D textures and UI elements. Correcting this fixes clipped 2D elements across several titles, including the fare meter in *Crazy Taxi* and character portraits in *The King of Fighters XI*.

Crazy Taxi:

<SliderCompare
  before={require("./img/comparisons/13823/CrazyTaxi_Before.webp").default}
  after={require("./img/comparisons/13823/CrazyTaxi_After.webp").default}
/>

The King of Fighters XI:

<SliderCompare
  before={require("./img/comparisons/13823/KOFXI_Before.webp").default}
  after={require("./img/comparisons/13823/KOFXI_After.webp").default}
/>
...
Irem games like to use dirty tricks to split out and select individual channels from the depth buffer for post-processing. The previous CRC hack missed certain channel setups and applied scanmasks that broke things in PCSX2. Providing the correct channels and skipping problematic scanmasks restores proper lighting, sky colors, and water effects in *Raw Danger! (Zettai Zetsumei Toshi 2)* and *PachiPara 12*.
...
Deletes render targets that get reduced to zero height during preloading instead of leaving them in the texture cache.

Preloading was adjusting the height of older targets in memory, leaving them with zero height. Subsequent draws were then mistakenly picking these empty targets as texture sources and failing to update from memory. Deleting empty targets fixes missing textures in *PachiPara 13*.
...
<SliderCompare
  ...
  after={require("./img/comparisons/14546/BOF_DragonQuarter_After.webp").default}
/>
...
Disallows GS local-to-local transfers from reversing copy directions when the destination area overlaps and overwrites source data.

On native PlayStation 2 hardware, local memory transfers are page-buffered, preventing self-overwriting hazards. In emulation, reversed transfer coordinates caused destination writes to clobber source data before it could be read, resulting in heavily corrupted textures. Restricting reversed copy directions fixes corrupted lighting and textures in *Armored Core 2*.
...
Updates render targets linked to an existing texture source when the target is dirty and overlaps the requested read.

If a texture source was already linked to a target that later received new data through an EE-to-GS upload, the texture cache wasn't updating that target, causing old data to be used. Updating the dirty linked target fixes corrupted arrow sprites in *Pump It Up: Exceed*.
...
Checks that the valid drawn area overlaps the requested read area when an exact target match is found.
...
Refactors `GSUploadQueue` to use the transfer type for clears instead of a separate variable.
...
Resets DC DCT (Discrete Cosine Transform) decoding precision in the IPU (Image Processing Unit) during soft resets.

Leftover DCT precision state was persisting across soft resets, corrupting MPEG video decoding on subsequent playback. Clearing this precision state on reset fixes severe screen flashing during FMV video playback in *Critical Velocity*.
